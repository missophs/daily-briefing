...
2. Convert Eastern time → UTC: add 4h (EDT, summer) or 5h (EST, winter)
...
These errors mean the Gmail refresh token expired. Fix:
...
3. Your browser shows an error page — that's expected
...
**Important:** `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` must match the `gmail_credentials.json` file used to generate the token. If you're not sure, check:
